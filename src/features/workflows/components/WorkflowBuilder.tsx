...
import { BracesIcon, Layers2Icon, ListTreeIcon, PanelRightCloseIcon, PlayIcon, PlusIcon, Settings2Icon, SparklesIcon, XIcon } from "lucide-react"
...
export function WorkflowBuilder({ modelOptions = [], onBack, project }: { modelOptions?: ModelOption[]; onBack: () => void; project: string }) {
  ...
  const resourcePlannerTarget = builder.workflow.nodes.find((node) => node.id === resourcePlannerTargetID) ?? null
  ...
  function openResourcePlanner(nodeID: string) {
    ...
    setNodeDetailOpen(false)
    setResourcePlannerTargetID(nodeID)
  }

  function applyResourcePlan(resource: PlannedResource) {
    const node = resourcePlannerTargetID ? builder.workflow.nodes.find((candidate) => candidate.id === resourcePlannerTargetID) : null
    if (!node || !node.type.startsWith("resource.")) return
    const data = node.data as ResourceNodeData
    const nextData: ResourceNodeData = {
      ...data,
      ...(resource.name?.trim() ? { name: resource.name.trim() } : {}),
    ...
    if (resource.config !== undefined) delete nextData.content
    if (resource.content !== undefined && node.type === "resource.plugin") delete nextData.config
    updateNode({
      ...node,
      data: nextData,
    } as WorkflowNode)
    setSelectedNodeID(node.id)
    setSelectedEdgeID(null)
    toastManager.add({ id: `workflow-resource-planner-applied-${Date.now()}`, title: "Resource 已生成", description: `已更新 ${getWorkflowNodeTitle(node)}；請儲存並重新測試發布 Workflow。`, type: "success" })
  }
      ...
      <WorkflowConfirmDialog action={requestedAction} busy={busy} name={builder.workflow.name} onConfirm={confirmAction} onOpenChange={(open) => { if (!open && !busy) setRequestedAction(null) }} scope={builder.workflow.scope} />
       <WorkflowPublishReport onOpenChange={setPublishReportOpen} open={publishReportOpen} report={builder.publishReport} />
          ...
          <WorkflowGeneratorDialog onCreateWorkflow={builder.createGenerated} onOpenChange={setWorkflowGeneratorOpen} open={workflowGeneratorOpen} project={workflowProject} />
           <WorkflowUpdaterDialog key={`workflow-updater:${builder.workflow.id}:${workflowUpdaterOpen ? "open" : "closed"}`} onApplyWorkflow={applyWorkflowUpdate} onOpenChange={setWorkflowUpdaterOpen} open={workflowUpdaterOpen && !protectedWorkflow} project={workflowProject} workflow={builder.workflow} workspace={project} />
