        ...
        <ul className="grid w-full max-w-full min-w-0 gap-2">
          {items.map((workflow) => (
            <li className={`flex w-full max-w-full min-w-0 items-center gap-3 overflow-hidden rounded-xl border p-3 ${workflow.id === activeWorkflowID ? "border-foreground/25 bg-accent" : "border-border"}`} key={`${workflow.scope}-${workflow.project ?? "global"}-${workflow.id}`}>
              ...
              <div className="min-w-0 flex-1"><div className="flex items-center gap-2"><strong className="truncate text-sm">{workflow.name}</strong><Badge size="sm" variant={workflow.scope === "global" ? "warning" : "secondary"}>{scopeLabel(workflow.scope)}</Badge><Badge size="sm" variant={workflow.protected ? "info" : "secondary"}>{workflow.protected ? "系統預設" : "非系統預設"}</Badge></div><p className="mt-0.5 truncate text-muted-foreground text-xs">{workflow.description || workflow.id}</p>{workflow.protected && <p className="mt-0.5 text-info-foreground text-[11px]">系統預設，唯讀且僅限 workflow-test</p>}<p className="mt-1 flex items-center gap-1 font-mono text-[10px] text-muted-foreground"><Clock3Icon aria-hidden="true" className="size-3" />{formatDate(workflow.updatedAt)}</p></div>
              ...
              <Button aria-label={workflow.protected ? `${workflow.name} 不可刪除` : `刪除 ${workflow.name}`} disabled={busy || workflow.protected} onClick={() => setDeleteTarget(workflow)} size="icon" title={workflow.protected ? "系統預設 Workflow 為唯讀且不可刪除" : undefined} variant="ghost"><Trash2Icon aria-hidden="true" /></Button>
            </li>
          ))}
