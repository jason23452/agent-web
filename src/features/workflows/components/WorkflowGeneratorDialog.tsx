...
export function WorkflowGeneratorDialog({ onCreateWorkflow, onOpenChange, open, project }: {
  ...
  async function generateWorkflow() {
    ...
    const text = [
      ...
      `目前 UI project: ${project ?? "global workflow"}`,
      "",
      request.trim(),
    ].join("\n")
    try {
      const response = await runWorkflowSystemCommand(WORKFLOW_GENERATOR_WORKFLOW_ID, text, controller.signal)
      ...
      const validation = await validateWorkflow(parsed, { signal: controller.signal })
  ...
  async function createWorkflow() {
    ...
    setError(null)
    try {
      const validation = await validateWorkflow(generated)
      if (!validation.valid || !validation.workflow) {
  ...
  return (
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogPopup className="h-[min(820px,calc(100dvh-2rem))] max-w-4xl" closeProps={{ "aria-label": "關閉 Workflow Generator" }}>
        <DialogHeader className="border-border border-b"><div className="flex items-start gap-3 pr-12"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-primary/8 text-primary"><SparklesIcon aria-hidden="true" className="size-5" /></span><div className="min-w-0"><DialogTitle>依需求建立 Workflow</DialogTitle><DialogDescription className="mt-1">先由官方 plan 規劃，再產生完整 V3 JSON；通過驗證後自動建立 Workflow。</DialogDescription></div></div><div className="mt-3 flex flex-wrap items-center gap-1.5 text-[11px]"><Badge variant="info">/workflow-generator</Badge>{project ? <Badge variant="secondary">Project · {project}</Badge> : <Badge variant="secondary">Global</Badge>}</div></DialogHeader>
