...
function uniqueEdgeID(candidate: string, existing: Set<string>): string {
  const base = candidate.replace(/[^A-Za-z0-9_-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 96) || "edge"
  let id = base
  let suffix = 2
...
function scopeWorkflowToProject(workflow: WorkflowV1, project: string): WorkflowV1 {
  return {
    ...
    nodes: workflow.nodes.map((node) => {
      ...
      return data.mode === "managed"
        ? { ...node, data: { ...data, scope: "project" } } as WorkflowNodeWithProjectResource
        : node
    }),
  }
}
