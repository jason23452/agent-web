...
import { ModalShell } from "@/shared/components/ui/modal-shell";
...
const initialSkillSettings: SkillDefinition[] = availableSkills.map(
  (skill, index) => ({
    id: skill,
    name: skill,
    description:
...
export function AppSidebar({
  ...
  const [skillInstallResult, setSkillInstallResult] = useState<{
    status: "success" | "error";
    message: string;
  } | null>(null);
            ...
            <div className="flex h-14 items-center justify-between gap-4 border-border/70 border-b px-4">
              <div className="flex min-w-0 items-center gap-2">
                ...
                <h2 className="font-semibold text-base">
                  {projectDialogView === "list" ? "打開項目" : "建立專案"}
                </h2>
              </div>
              <div className="flex items-center gap-1.5">
                {projectDialogView === "list" && (
                  <Button
                    onClick={() => setProjectDialogView("create")}
                    size="sm"
                    variant="outline"
                  >
      ...
      <ModalShell
        ...
        footer={
          !selectedModelProvider && (
            <>
              <p className="text-muted-foreground text-xs">
                建議使用 `/connect` 或 {"{env:KEY}"}，避免把明文 API Key
                寫入專案。
              </p>
              <Button onClick={() => setUserSettingsOpen(false)} size="lg">
                完成
              </Button>
            </>
          )
        }
        maxWidth={selectedModelProvider ? "max-w-[640px]" : "max-w-[760px]"}
        onOpenChange={(open) => {
          if (!open) closeUserSettings();
        }}
        open={userSettingsOpen}
        title={!selectedModelProvider ? "使用者設定" : undefined}
      >
        {!selectedModelProvider ? (
          <div className="mx-auto grid max-w-[680px] gap-8">
            ...
            >
              ...
              <ul className="grid gap-2">
                {modelProviders
                  ...
                  .map((provider) => (
                    <li
                      className="flex min-h-14 items-center justify-between gap-3 rounded-lg bg-muted/50 px-4 py-3"
                      key={provider.id}
                    >
            ...
            <section
              className="grid gap-3"
              aria-labelledby="popular-provider-title"
            >
              <h3 className="font-semibold text-sm" id="popular-provider-title">
                熱門提供商
              </h3>
          ...
          <div className="grid gap-8 px-0 pt-1 pb-14">
            ...
            >
              ...
              >
                ...
                並輸入以下代碼，以連接你的帳戶並在 OpenCode 中使用{" "}
                {selectedModelProvider.name} 模型。
              </p>
          ...
          <div className="grid gap-8 px-0 pt-1 pb-14">
            ...
            >
              ...
              >
                選擇 {selectedModelProvider.name} 的登錄方式。
              </p>
                      ...
                      <div className="flex items-center gap-3 rounded-lg bg-muted/55 px-4 py-3">
                        ...
                        <Menu>
                          ...
                          <MenuPopup align="end" className="min-w-32">
                            <MenuItem onClick={() => openEditMcpServer(server)}>
                              編輯
                            </MenuItem>
                            <MenuItem
                              onClick={() => setDefaultMcpServer(server.id)}
                            >
                              設為默認
                            </MenuItem>
                            <MenuSeparator />
      ...
      <ModalShell
        ...
        footer={
          <>
            ...
            <div className="flex items-center gap-2">
              <Button
                onClick={() => setPluginSkillDialogOpen(false)}
                size="lg"
                variant="outline"
              >
                關閉
              ...
              <Button
                disabled={!pluginSkillHasChanges}
                onClick={() => confirmBatchUpdate("plugins-skills")}
                size="lg"
              >
                更新
              </Button>
        ...
        headerActions={
          pluginSkillDialogView === "list" && (
            <Button
              onClick={() =>
                setPluginSkillDialogView(
                  pluginSkillTab === "plugins" ? "add-plugin" : "add-skill",
                )
              }
              size="sm"
              variant="outline"
        ...
        onOpenChange={(open) => {
          setPluginSkillDialogOpen(open);
          if (!open) setPluginSkillDialogView("list");
        }}
        open={pluginSkillDialogOpen}
        title={
          pluginSkillDialogView === "add-plugin"
            ? "新增 Plugin"
            : pluginSkillDialogView === "add-skill"
              ? "新增 Skill"
              : "外掛/技能"
        }
      >
          ...
          {pluginSkillDialogView === "list" ? (
            <>
              <div className="grid grid-cols-2 rounded-lg bg-muted p-1">
                <button
                  className={`h-8 rounded-md font-medium text-sm transition ${pluginSkillTab === "plugins" ? "bg-background text-foreground shadow-xs/5" : "text-muted-foreground hover:text-foreground"}`}
                  onClick={() => {
                    setPluginSkillTab("plugins");
                    setPluginSkillSearch("");
                  }}
                ...
                <button
                  className={`h-8 rounded-md font-medium text-sm transition ${pluginSkillTab === "skills" ? "bg-background text-foreground shadow-xs/5" : "text-muted-foreground hover:text-foreground"}`}
                  onClick={() => {
                    setPluginSkillTab("skills");
                    setPluginSkillSearch("");
                  }}
              ...
              <InputGroup data-size="sm">
                ...
                <InputGroupInput
                  aria-label="搜尋外掛或技能"
                  onChange={(event) => setPluginSkillSearch(event.target.value)}
                  placeholder={
                    pluginSkillTab === "plugins"
                      ? "搜尋外掛名稱、描述或 entry"
                      : "搜尋技能名稱、描述或路徑"
                  }
                  value={pluginSkillSearch}
                />
                {pluginSkillSearch && (
                  <InputGroupAddon align="inline-end">
                    <button
              ...
              {pluginSkillTab === "plugins" && (
                <section
                  ...
                  aria-labelledby="plugins-settings-title"
                >
                  <div className="flex items-center justify-between gap-3">
                    <h3
                      className="font-semibold text-sm"
                      id="plugins-settings-title"
                    >
                      Plugins
                    </h3>
                    <Badge size="sm" variant="secondary">
                  ...
                  <ul className="grid gap-2">
                    {filteredPlugins.map((plugin) => (
                      <li
                        className="rounded-lg bg-muted/55 px-4 py-3"
                        key={plugin.id}
                      >
                          ...
                          <div className="min-w-0">
                            <div className="flex min-w-0 flex-wrap items-center gap-2">
                              ...
                              <Badge
                                size="sm"
                                variant={
                                  plugin.enabled ? "success" : "secondary"
                                }
                              >
              ...
              {pluginSkillTab === "skills" && (
                <section
                  className="grid gap-2"
                  aria-labelledby="skills-settings-title"
                >
                  <div className="flex items-center justify-between gap-3">
                    <h3
                      className="font-semibold text-sm"
                    ...
                    <Badge size="sm" variant="secondary">
                      {skillSettings.filter((skill) => skill.enabled).length}{" "}
                      enabled
                    </Badge>
                  </div>
                  <ul className="grid gap-2">
                      ...
                      >
                        <div className="min-w-0">
                          <div className="flex min-w-0 flex-wrap items-center gap-2">
                            ...
                            <Badge
                              size="sm"
                              variant={skill.enabled ? "success" : "secondary"}
                            >
            ...
            <div className="grid gap-4 rounded-xl bg-muted/35 p-5">
              <div className="grid gap-1">
                ...
                <p className="text-muted-foreground text-xs leading-5">
                  依 OpenCode 官方方式載入：npm 寫入 config，local/archive 放到
                  plugins 目錄。
                </p>
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <label className="grid gap-1 text-muted-foreground text-xs">
                  ...
                  <select
                    className="h-9 rounded-lg border border-input bg-background px-2 text-sm outline-none focus:ring-2 focus:ring-ring"
                    disabled={pluginForm.method === "npm"}
                    onChange={(event) =>
                      setPluginForm((current) => ({
                        ...current,
                        installTarget: event.target.value as
                          | "project"
                          | "global",
                      }))
                    }
                    value={pluginForm.installTarget}
                  >
              ...
              {pluginForm.method === "archive" ? (
                ...
                <label className="grid gap-1 text-muted-foreground text-xs">
                  {pluginForm.method === "npm" ? "NPM package" : "Plugin name"}
                  <Input
              ...
              {pluginForm.method === "archive" && pluginForm.archiveName && (
                <p className="truncate text-muted-foreground text-xs">
                  已選擇：{pluginForm.archiveName}
                </p>
              )}
              {pluginInstallResult && (
                <div
                  className={`rounded-md border px-3 py-2 text-xs ${pluginInstallResult.status === "success" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-red-200 bg-red-50 text-red-700"}`}
                >
              ...
              <p className="text-muted-foreground text-xs">
                ...
                `~/.config/opencode/plugins/`；archive: 解壓到 plugin
                directory。
              </p>
              <div className="flex justify-end gap-2">
                <Button
                ...
                <Button
                  onClick={addPluginFromOfficialSource}
                  size="sm"
                  type="button"
                >
                  新增 Plugin
            ...
            <div className="grid gap-4 rounded-xl bg-muted/35 p-5">
              <div className="grid gap-1">
                ...
                <p className="text-muted-foreground text-xs leading-5">
                  依 OpenCode 官方方式建立 name/SKILL.md，可放在
                  OpenCode、Claude 或 Agents 相容目錄。
                </p>
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="grid gap-1 text-muted-foreground text-xs">
              ...
              {skillForm.archiveName && (
                <p className="truncate text-muted-foreground text-xs">
                  已選擇：{skillForm.archiveName}
                </p>
              )}
              {skillInstallResult && (
                <div
                  className={`rounded-md border px-3 py-2 text-xs ${skillInstallResult.status === "success" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-red-200 bg-red-50 text-red-700"}`}
                >
                  {skillInstallResult.message}
                </div>
              )}
              <p className="text-muted-foreground text-xs">
                官方規則：名稱需符合 `^[a-z0-9]+(-[a-z0-9]+)*$`，目錄名需與
                SKILL.md frontmatter 的 `name` 一致。
              </p>
              <div className="flex justify-end gap-2">
                <Button
                  onClick={() => setPluginSkillDialogView("list")}
                ...
                <Button
                  onClick={addSkillFromOfficialSource}
                  size="sm"
                  type="button"
                >
                  新增 Skill
      ...
      <ModalShell
          ...
          <>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                onClick={() => confirmBatchUpdate("agents-tools")}
                size="lg"
              >
                更新
              </Button>
        ...
        headerActions={
          <button
            ...
            className="grid size-8 place-items-center rounded-lg text-muted-foreground hover:bg-accent hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            onClick={
              agentToolTab === "tools" ? openAddToolMode : openAddAgentMode
            }
            type="button"
          >
      ...
      >
                          ...
                          <div className="group flex items-start gap-3 rounded-lg bg-muted/55 px-3 py-3 transition-colors hover:bg-accent">
                            ...
                            <Menu>
                              ...
                              <MenuPopup align="end" className="min-w-36">
                                ...
                                <MenuItem
                                  onClick={() => openEditAgentMode(agent)}
                                >
                                  編輯
                                </MenuItem>
                      ...
                      <div className="group flex items-start gap-3 rounded-lg bg-muted/55 px-3 py-3 transition-colors hover:bg-accent">
                        ...
                        <Menu>
                          ...
                          <MenuPopup align="end" className="min-w-36">
                            ...
                            {tool.source === "custom" && (
                              <MenuItem onClick={() => openEditToolMode(tool)}>
                                編輯
                              </MenuItem>
                            )}
          ...
          <div className="grid min-h-0 flex-1 gap-4 overflow-y-auto px-6 pb-6">
            <div className="grid gap-4 rounded-lg bg-muted/45 p-5">
                    ...
                    onChange={(event) => {
                      ...
                      setToolForm((current) => ({
                        ...
                        runtime: event.target
                          .value as ToolDefinition["runtime"],
                        entry:
                          current.entry ||
              ...
              <div className="rounded-lg border border-dashed bg-background px-3 py-3 text-muted-foreground text-xs">
                自訂工具會出現在工具清單與 Agent tool
                selector。內建工具不能編輯，只有 custom tool 可以編輯或刪除。
              </div>
            </div>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className="text-muted-foreground text-xs">
                Runtime 目前支援 Python 與 JS/TS；請先通過 Tool Call Test
                再保存。
              </p>
              <Button
                disabled={
                  !toolForm.name.trim() || toolTestResult?.status !== "success"
                }
                onClick={submitToolConfig}
              >
          ...
          <div className="grid min-h-0 flex-1 gap-4 overflow-y-auto px-6 pb-6">
            <div className="grid grid-cols-2 rounded-lg bg-muted p-1">
              <button
                ...
                onClick={() => switchAgentConfigMode("interface")}
                type="button"
              >
                介面配置
              </button>
              <button
                ...
                onClick={() => switchAgentConfigMode("yaml")}
                type="button"
              >
                文字配置
              </button>
            ...
            <div className="rounded-lg bg-muted/55 p-4">
              <div className="flex min-w-0 flex-wrap items-center gap-2">
                ...
                <Badge
                  size="sm"
                  variant={
                    selectedAgent.scope === "system" ? "info" : "success"
                  }
                >
                  {selectedAgent.scope}
                </Badge>
            ...
            {agentConfigMode === "interface" ? (
              <>
                        ...
                        >
                          <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto_7rem_auto] sm:items-center">
                            ...
                            <select
                              ...
                              disabled
                              value={
                                typeof selectedAgent.permission[
                                  permissionKey
                                ] === "string"
                ...
                >
                          ...
                          {guidanceSubagent === subagentId && (
                            <label className="grid gap-1.5 border-border/70 border-t pt-2 text-muted-foreground text-xs">
                              ...
                              <Textarea
                                ...
                                rows={3}
                                value={
                                  selectedAgent.subagentGuidance?.[
                                    subagentId
                                  ] ?? ""
                                }
                              />
                  ...
                  <p className="text-muted-foreground text-xs">
                    查看設定為唯讀；請到自訂 Agent 的編輯頁修改 callable
                    subagents。
                  </p>
                  <p className="text-muted-foreground text-xs">
                    這會輸出成官方 permission.task
                    規則；清單會排除自己與會造成回呼循環的 agent。
                  </p>
                </section>
              ...
              <section
                className="grid gap-2"
                aria-labelledby="agent-yaml-preview-title"
              >
                <h4
                  className="font-semibold text-sm"
                  id="agent-yaml-preview-title"
                >
                  Markdown 預覽
                </h4>
            ...
            <div className="flex justify-end">
              {selectedAgent.scope === "custom" ? (
                <Button
                  onClick={() => openEditAgentMode(selectedAgent)}
                  size="sm"
                >
                  編輯 Agent
                </Button>
        ...
        {agentDialogView === "config" && (
          <div className="grid min-h-0 flex-1 gap-4 overflow-y-auto px-6 pb-6">
              ...
              <div className="grid gap-4 rounded-lg bg-muted/45 p-5">
                      ...
                      onChange={(event) =>
                        setAgentForm((current) => {
                          const mode = event.target
                            .value as AgentDefinition["mode"];
                          return {
                            ...current,
                            mode,
                            hidden:
                              mode === "subagent" ? current.hidden : false,
                          };
                        })
                      }
                    ...
                    <label className="grid gap-2 text-muted-foreground text-sm">
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          setAgentForm((current) => ({
                            ...current,
                            promptSource: event.target
                              .value as AgentDefinition["promptSource"],
                          }))
                        }
                        ...
                        >
                          <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto_7rem_auto] sm:items-center">
                            ...
                            <select
                              ...
                              value={
                                typeof agentForm.permission[permissionKey] ===
                                "string"
                          ...
                          {guidanceTool === tool && isCustomToolName(tool) && (
                            <label className="grid gap-1.5 border-border/70 border-t pt-2 text-muted-foreground text-xs">
                              ...
                              <Textarea
                                aria-label={`${tool} 使用情境`}
                                onChange={(event) =>
                                  updateToolGuidance(tool, event.target.value)
                                }
                            ...
                            : {
                                ...
                                toolGuidance: isCustomToolName(toolToAdd)
                                  ? {
                                      ...current.toolGuidance,
                                      [toolToAdd]:
                                        current.toolGuidance[toolToAdd] ?? "",
                                    }
                                  : current.toolGuidance,
                                permission: {
                                  ...current.permission,
                ...
                <section className="grid gap-2">
                        ...
                        >
                          <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto_auto_auto] sm:items-center">
                            ...
                            <button
                              ...
                              className="grid size-6 place-items-center rounded-sm text-muted-foreground hover:bg-accent hover:text-foreground"
                              onClick={() => removeFormSubagent(subagentId)}
                              type="button"
                            >
                              <XIcon aria-hidden="true" className="size-3" />
                            </button>
                          ...
                          {guidanceSubagent === subagentId && (
                            <label className="grid gap-1.5 border-border/70 border-t pt-2 text-muted-foreground text-xs">
                              ...
                              <Textarea
                                ...
                                rows={3}
                                value={
                                  agentForm.subagentGuidance[subagentId] ?? ""
                                }
                              />
                            </label>
                          )}
                  ...
                  <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
                    <select
                      className="h-8 rounded-lg border border-input bg-background px-2 text-sm outline-none focus:ring-2 focus:ring-ring"
                      onChange={(event) => setSubagentToAdd(event.target.value)}
                      value={subagentToAdd}
                    >
                      {getCallableSubagentOptions(
                        editingAgentId,
                        agentForm.subagents,
                      ).map((agent) => (
                  ...
                  <p className="text-muted-foreground text-xs">
                    這會輸出成官方 permission.task 規則；subagent 可以再調用其他
                    subagent，但不能選回呼叫鏈上的 agent。
                  </p>
                </section>
                        ...
                        {guidanceSkill === skill && (
                          <label className="grid gap-1.5 border-border/70 border-t pt-2 text-muted-foreground text-xs">
                            ...
                            <Textarea
                              aria-label={`${skill} 使用情境`}
                              onChange={(event) =>
                                updateSkillGuidance(skill, event.target.value)
                              }
                          ...
                          current.skills.includes(skillToAdd)
                            ...
                            : {
                                ...
                                skillGuidance: {
                                  ...current.skillGuidance,
                                  [skillToAdd]:
                                    current.skillGuidance[skillToAdd] ?? "",
                                },
                              },
                        )
                      }
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className="text-muted-foreground text-xs">
                介面配置適合快速新增；文字配置可直接編輯 opencode agent
                Markdown。
              </p>
              <Button
                disabled={
