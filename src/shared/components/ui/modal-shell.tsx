...
export function ModalShell({
  ...
  return (
    <Dialog onOpenChange={onOpenChange} open={open}>
      ...
      >
        <div className={cn("flex h-14 shrink-0 items-center justify-between gap-4 px-5", headerClassName)}>
          <div className="flex min-w-0 items-center gap-2">
            ...
            {(title || description) && (
              <div className="min-w-0">
                {title && <DialogTitle className="font-sans text-base leading-6">{title}</DialogTitle>}
        ...
        {footer && (
          <div className={cn("flex shrink-0 flex-wrap items-center justify-between gap-3 border-border/70 border-t bg-background px-6 py-4", footerClassName)}>
            {footer}
          </div>
        )}
      </DialogPopup>
